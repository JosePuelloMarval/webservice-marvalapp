...
export const createPayment = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const accountStatus = await AccountStatus.findOne({ where: { id: accountStatusId } });
    if (!accountStatus) {
    ...
    payment.accountStatus = accountStatusId;
    payment.user = userId;
    ...
    res.status(201).json({ message: 'Pago registrado correctamente', id: payment.id });
  } catch (error) {
